...
| `NavigationLink` | Type-safe `navController.navigate()` with `@Serializable` routes |
...
NavHost(navController, startDestination = Home) {
    ...
    composable<Details> { backStackEntry ->
        ...
        DetailScreen(detailId = route.id)
    }
}
```
...
### 6. String-Based Navigation Routes

**Pitfall:** Using string-based routes like `composable("details/{id}")` which are error-prone and not type-safe.

**Solution:** Use Navigation Compose 2.8+ with `@Serializable` route classes. Compile-time type safety eliminates route typos and argument parsing bugs.
